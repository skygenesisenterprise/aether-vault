...
run_server() {
    ...
    if [ "${NODE_ENV}" = "production" ]; then
        log_info "Aether Vault frontend starting (static)"
    ...
    log_info "Aether Vault frontend starting (development)"
    log_info "Frontend listening on 0.0.0.0:${FRONTEND_PORT}"
...
run_worker() {
    ...
    log_info "Aether Vault API starting"
...
run_air() {
    ...
    log_info "Aether Vault API starting (hot-reload)"
